...
| 节点名称 | 功能描述 |
| --- | --- |
| **🎵 加载单个音频** | 支持 `.wav`, `.mp3` 等格式，**带弹窗浏览按钮**，支持截取音频片段。 |
| **📂 批量加载音频文件夹** | 一键读取文件夹内所有音频，支持递归搜索子目录，**带弹窗浏览按钮**。 |
...
1. 添加节点 **“📂 批量加载音频文件夹”** -> 点击 **“浏览文件夹”** 选择你的素材目录。
2. 连接到 **“🎤 Qwen 批量语音识别”** 节点。
3. (可选) 在 **“提示词”** 中输入 `转录为简体中文，忽略背景杂音`。
4. 点击 **Queue Prompt**，查看控制台进度条，等待结果输出。
